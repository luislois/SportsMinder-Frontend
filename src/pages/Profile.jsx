...
const Profile = () => {
  ...
  const handleResetPassword = () => {
    modal.confirm({
        ...
        cancelText: 'Cancel',
        async onOk(){
        const response = await fetch('http://dev-fhvg4rnmgf8fdbv6.us.auth0.com/dbconnections/change_password', {
              method: 'POST',
              headers: {
  ...
  const handleDeleteReserve = (booking) => {
    ...
    const content = bookingPlusDays <= maxDay ? 'You do not recieve a refund.' : 'You will recieve a refund in a few days.';
      ...
      modal.confirm({
        ...
        cancelText: 'Cancel',
        async onOk(){
          const response = await fetch(`localhost:8080/api/bookings/${bookingGeneratedId}`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json'
            },
  ...
  const fetchData = useCallback(async () => {
    if(user){
      ...
      const data = await fetch(`localhost:8080/api/bookings/user/${user.dni}`);
